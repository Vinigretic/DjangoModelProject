...
# Этот способ можно применять когда существущие данные не нужно извлекать, а просто изменить

# Person.objects.all().update(name = 'Cat')

# Четвертый способ
# Для него необходимо использовать импорт, который позволяет получать значение из БД для его изменения
